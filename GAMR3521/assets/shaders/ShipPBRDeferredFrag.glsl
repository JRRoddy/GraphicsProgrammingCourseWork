...
uniform float u_baseReflectivity;

vec3 normalFromMap = texture(u_normalMap, texCoord).rgb;
vec3 normal = normalize(tangentToWorld * (normalFromMap * 2.0 - 1.0));

float metal = texture(u_metalMap,texCoord).r;
float roughness = texture(u_roughMap,texCoord).r;

void main()
{
     

     g_position = vec4(fragmentPos,metal); 

     g_normal = vec4(normal,roughness);
     
     vec3 diffuse = texture(u_albedoMap ,texCoord).rgb;
     float specular = texture(u_specularMap,texCoord).r;
     
     g_diffSpec = vec4(diffuse,u_baseReflectivity);
